(ns agronomyops.operation
  "OperationActor -- one agronomy operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (AgronomyOps-LLM)
  is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Agronomy Governor (:govern) and the rollout phase
  ...
  One graph run = one agronomy operation (intake -> advise -> govern ->
  ...
  decision to a human operator (agronomy operator/extension officer). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:sample/collect`/`:treatment/apply` ALWAYS reach this
  node when the governor is clean -- see `agronomyops.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [agronomyops.agronomyopsllm :as agronomyopsllm]
            [agronomyops.governor :as governor]
...
(defn build
  ...
  `agronomyops.store/Store`).
  opts:
    :advisor      -- an `agronomyops.agronomyopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (agronomyopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; AgronomyOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (agronomyopsllm/-advise advisor store request)]
            {:proposal p :audit [(agronomyopsllm/trace request p)]})))

      ;; Agronomy Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
